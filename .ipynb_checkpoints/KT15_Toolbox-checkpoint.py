...
def KT15_labcalibration(data_folder, output_path, caltemps):
    ...
    seaCal = pd.DataFrame(data=None,index=caltemps,columns=['Mean','Std','Housing'], dtype='float')
    skyCal = pd.DataFrame(data=None,index=caltemps,columns=['Mean','Std','Housing'], dtype='float')
    ...
    for filepath in files:
        ...
        seaCal.Housing.loc[bb_temp] = kt.SeaRef.mean()
        
        skyCal.Mean.loc[bb_temp] = kt.SkyTemp.mean()
        skyCal.Std.loc[bb_temp] = kt.SkyTemp.std()
        skyCal.Housing.loc[bb_temp] = kt.SkyRef.mean()
...
def KT15_calibrate_skycorrect82(l1data_path, output_path, cal_path):
    ...
    ######################################################################################################
def KT15_skycorrect82(l1data_path, output_path):
    '''
    This function takes data from a pair of up- and down-looking KT15.82 8um-14um infrared radiometers and calculates a final value for the 'true' SST corrected for reflections from the sky, following SOURCE.
    ...
    fig,axx = plt.subplots(facecolor='w')
    kt.SST.plot(ax=axx)
    plt.title(f'KT-15 Sky-Corrected Data: {kt.experiment}')
    #plt.savefig(output_path+f'/{kt.experiment}_KT15_CalibratedSST_v2.png')

    #save the final dataframe in the output path
    kt.to_netcdf(output_path+f'/{kt.experiment}_KT15_SkyCorrectedSST_only.cdf')

    ######################################################################################################
    
    ######################################################################################################
# VERSION CONTROL IN CASE I MESS SOMETHING UP, THIS WAS THE ORIGINAL:
def KT15_labcalibration_old(data_folder, output_path, caltemps):
    '''
    This function takes KT15 data from a blackbody calibration done in the lab, and calculates a linear fit to the measured values given the blackbody reference temperature. To apply the calibration calculated by this function, apply this formula:
        calibrated_data = (raw_data - yint)/slope
